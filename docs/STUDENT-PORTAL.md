...
5. Open `public/upgrade.php`, confirm the backup and run the additive upgrade. It adds `portal_accounts`, `portal_otp_challenges` and `portal_events`; existing CRM tables/data remain in place. Re-running is safe. A database account with schema-creation/index permissions is needed during upgrade. Native MySQL DDL is not fully transactional; if it fails part-way, fix permissions and rerun rather than deleting data.
...
The portal is **read-only**, not an online payment gateway or student-registration system. It exposes no enquiry notes, staff directory, other student data, finance-management actions or raw email-queue data. Attendance/exam/results modules have not been implemented; the UI says so rather than showing fabricated records. The balance is admission fee minus recorded payments, not an installment due-date schedule. Existing receipts show their saved payment-time snapshot, not necessarily today's balance.
