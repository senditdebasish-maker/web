...
$tickerItems = [];
foreach ($notices as $n) { $tickerItems[] = 'Admissions ' . ($n['closing'] ? 'closing soon' : 'open') . ': ' . $n['course'] . ' — apply by ' . $n['closes']; }
if ($programCount) $tickerItems[] = $programCount . ' program' . ($programCount === 1 ? '' : 's') . ' open across ' . count($institutes) . ' ' . (count($institutes) === 1 ? 'campus' : 'campuses');
if ($brandPhone !== '') $tickerItems[] = 'Admission helpline: ' . $brandPhone;
if (!$tickerItems) $tickerItems = ['Admissions opening soon — contact the office', 'Welcome to ' . $brand];
$stats = [
...
<?=siteHeader($brand,$brandKind,$page,'<a class="u-btn ghost" href="'.e(siteHomeUrl().'?page=login').'">Sign In</a><a class="u-btn solid" href="'.e(sitePublicUrl('apply.php')).'">Apply Now</a>',$tickerItems)?>
<main class="u-main">
<?php if (!$ready): ?>
...
<section class="u-hero"><div class="u-hero-inner"><div class="u-eyebrow">TRADITION MEETS INNOVATION · ADMISSIONS OPEN</div><h1>Nurturing global leaders.</h1><p><?= e($brand) ?> offers <?= e($brandKind) ?> programs with verified admissions, caring faculty and a modern student portal. Create your account, apply online and track your application — all in one place.</p><div class="u-cta"><a class="u-btn solid big" href="public/apply.php">Apply for Admission →</a><a class="u-btn ghost big" href="?page=courses">Explore Programs</a><a class="u-btn ghost big" href="?page=login">Student / Office Sign In</a></div></div></section>
<div class="u-hero-badges"><?php foreach (siteAccreditations() as [$icon, $title, $sub]): ?><div class="u-badge-card"><span class="u-badge-icon"><?= $icon ?></span><div><strong><?= e($title) ?></strong><small><?= e($sub) ?></small></div></div><?php endforeach; ?></div>
...
<div class="u-side-card u-vc"><div class="u-side-title">Vice-Chancellor's Message</div><div class="u-vc-head"><span class="u-vc-avatar"><?= e(strtoupper(mb_substr($brand, 0, 1))) ?></span><strong><?= e($brand) ?></strong></div><blockquote>“<?= e(siteVcMessage($brand)) ?>”</blockquote><cite>Office of the Vice-Chancellor</cite></div>
<div class="u-side-card"><div class="u-side-title">Explore our programs</div><?php if (!$programs): ?><p class="u-muted">Programs open here when admissions begin.</p><?php else: ?><ul><?php foreach (array_slice($programs, 0, 4) as $p): ?><li><a href="public/apply.php?page=course&course=<?= (int)$p['id'] ?>"><?= e($p['name']) ?><small><?= e($p['institute_name']) ?></small></a></li><?php endforeach; ?></ul><p class="u-more"><a href="?page=courses">All programs →</a></p><?php endif; ?></div>
<div class="u-side-card"><div class="u-side-title">Top recruiters</div><div class="u-recruiters"><?php foreach (siteRecruiters() as [$icon, $name]): ?><span><?= $icon ?> <?= e($name) ?></span><?php endforeach; ?></div></div>
<div class="u-side-card"><div class="u-side-title">Reach us</div><p><strong><?= e($brand) ?></strong></p><?php if (trim($brandAddress) !== ''): ?><p>📍 <?= e($brandAddress) ?><?= $brandCity ? ', ' . e($brandCity) : '' ?></p><?php elseif ($brandCity): ?><p>📍 <?= e($brandCity) ?></p><?php endif; ?><?php if ($brandPhone !== ''): ?><p class="u-phone">☎ <?= e($brandPhone) ?></p><?php endif; ?><p class="u-more"><a href="?page=contact">All campuses →</a></p></div>
</aside></div>
