...
function uDocHead(string $title, string $brand): void {
    ...
    echo '<!doctype html><html lang="' . e($htmlLang) . '"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
        ...
        . '<noscript><style>body.future .f-reveal{opacity:1 !important;transform:none !important}</style></noscript></head><body class="future" data-page="' . e($page) . '"><a class="f-skip" href="#main">' . e(tr('Skip to content')) . '</a>';
}
?>
...
<div class="f-hero-cta"><a href="apply.php" class="f-btn primary big"><?= e(tr('Apply for Admission')) ?> →</a><a href="#programs" class="f-btn ghost big"><?= e(tr('Explore Programs')) ?> ↓</a></div>
...
<section class="f-section alt"><div class="f-wrap f-duo">
...
<?php if (!$programs): ?><p class="f-muted"><?= e(tr('Admissions are opening soon.')) ?> <?= e(tr('Check')) ?> <a href="?page=notices"><?= e(tr('Notices')) ?></a> <?= e(tr('or contact the office.')) ?></p><?php else: ?><div class="f-grid-3"><?php foreach (array_slice($programs, 0, 6) as $p): $short = collegeDegreeShort($p['name']); $pdesc = trim((string)($p['description'] ?? '')); if ($pdesc === '') $pdesc = 'Quality pharmacy education with practical training, experienced faculty and research opportunities.'; ?><article class="f-prog-card f-reveal"><div class="f-prog-media"><img src="<?= e(collegeProgramImage($p['name'])) ?>" alt="<?= e($p['name']) ?>" loading="lazy"><span class="f-prog-tag"><?= e(tr('Pharmacy')) ?></span></div><div class="f-prog-body"><h3><?= e($short !== '' ? $short : $p['name']) ?></h3><?php if ($short !== ''): ?><h4><?= e($p['name']) ?></h4><?php endif; ?><div class="f-meta"><span><?= collegeIcon('clock') ?><?= e($p['duration']) ?></span><span><?= collegeIcon('flask') ?><?= e(tr('Practical')) ?></span></div><p><?= e(mb_substr($pdesc, 0, 110)) ?><?= mb_strlen($pdesc) > 110 ? '…' : '' ?></p><a class="f-link-arrow" href="student.php?page=course&course=<?= (int)$p['id'] ?>"><?= e(tr('Learn More')) ?> →</a></div></article><?php endforeach; ?></div><?php endif; ?>
</div></section>
...
<section class="f-section"><div class="f-wrap f-duo">
<div class="f-reveal"><div class="f-eyebrow"><?= e(tr('Latest notices')) ?></div><h2><?= e(tr('Notices & dates')) ?></h2>
<?php if (!$notices): ?><p class="f-muted"><?= e(tr('No admission notices right now.')) ?></p><?php else: ?><div class="f-notice-list"><?php foreach (array_slice($notices, 0, 5) as $n): $nts = strtotime((string)$n['closes']) ?: time(); ?><div class="f-notice"><div class="f-notice-date"><strong><?= e(date('d', $nts)) ?></strong><small><?= e(date('M', $nts)) ?></small></div><div><a href="student.php?page=course&course=<?= (int)$n['id'] ?>">Admissions <?= $n['closing'] ? 'closing soon' : 'open' ?>: <?= e($n['course']) ?></a><?php if ($n['closing']): ?> <span class="f-pill soon"><?= e(tr('NEW')) ?></span><?php endif; ?></div></div><?php endforeach; ?></div><?php endif; ?>
...
<section class="f-section"><div class="f-wrap"><div class="f-cta-band f-reveal"><div><h2><?= e(tr('Begin Your Journey in Pharmacy')) ?></h2><p>Applications are now open. Start your application online today.</p></div><div class="f-btn-row"><a href="apply.php" class="f-btn accent big"><?= e(tr('Apply Online Now')) ?> →</a></div></div></div></section>
...
<?php if (!$institutes): ?><p class="f-muted">Campus details will appear here once the office adds institutes.</p><?php else: ?><div class="f-grid-3"><?php foreach ($institutes as $i): ?><article class="f-card f-reveal"><div class="f-ico"><?= collegeIcon('pin') ?></div><h3><?= e($i['name']) ?></h3><p class="f-meta"><?= e($i['kind']) ?> · <?= e($i['city']) ?></p><?php if (trim($i['address']) !== ''): ?><p><?= e($i['address']) ?></p><?php endif; ?><p><a class="f-link-arrow" href="tel:<?= e(preg_replace('/[^+0-9]/', '', (string)$i['phone'])) ?>"><?= collegeIcon('phone') ?> <?= e($i['phone']) ?></a></p></article><?php endforeach; ?></div><?php endif; ?>
</div></section>
<?php elseif ($page === 'course' && $catalogCourse): ?>
<section class="f-wrap"><div class="f-crumbs f-reveal"><a href="?page=courses"><?= e(tr('Programs')) ?></a> › <?= e($catalogCourse['name']) ?></div>
...
<?php if (!$visiblePrograms): ?><p class="f-muted"><?= $courseQuery !== '' ? e(tr('No programs match your search.')) : e(tr('No active courses are available yet.')) ?></p><?php else: ?><div class="f-grid-3"><?php foreach ($visiblePrograms as $p): $short = collegeDegreeShort($p['name']); $open = (int)$p['admission_open'] === 1; ?><article class="f-prog-card f-reveal"><div class="f-prog-media"><img src="<?= e(collegeProgramImage($p['name'])) ?>" alt="<?= e($p['name']) ?>" loading="lazy"><?php if ($open): ?><span class="f-prog-tag"><?= e(tr('Open')) ?></span><?php endif; ?></div><div class="f-prog-body"><div class="f-eyebrow"><?= e($p['institute_name']) ?></div><h3><?= e($short !== '' ? $short : $p['name']) ?></h3><?php if ($short !== ''): ?><h4><?= e($p['name']) ?></h4><?php endif; ?><div class="f-meta"><span><?= collegeIcon('clock') ?><?= e($p['duration']) ?></span><span><?= collegeIcon('pin') ?><?= e($p['city']) ?></span></div><p><?= e(mb_substr((string)($p['description'] ?? ''), 0, 140)) ?><?= mb_strlen((string)($p['description'] ?? '')) > 140 ? '…' : '' ?></p><p class="f-fee">₹<?= number_format((int)$p['fee_minor'] / 100, 2) ?></p><?php if ($open): ?><p class="f-apply-by"><?= e(tr('Apply by')) ?> <?= e($p['closes_on']) ?></p><p><a class="f-btn primary" href="student.php?page=course&course=<?= (int)$p['id'] ?>"><?= e(tr('Details & Apply')) ?> →</a></p><?php else: ?><p><a class="f-btn ghost" href="?page=course&course=<?= (int)$p['id'] ?>"><?= e(tr('Details')) ?> →</a></p><?php endif; ?></div></article><?php endforeach; ?></div><?php endif; ?></section>
...
<?php if ($programs): ?><div class="f-table-wrap"><table><thead><tr><th><?= e(tr('Program')) ?></th><th><?= e(tr('Campus')) ?></th><th><?= e(tr('Fee')) ?></th><th><?= e(tr('Apply by')) ?></th><th></th></tr></thead><tbody><?php foreach ($programs as $p): ?><tr><td><strong><?= e($p['name']) ?></strong></td><td><?= e($p['institute_name']) ?></td><td>₹<?= number_format((int)$p['fee_minor'] / 100, 2) ?></td><td><?= e($p['closes_on']) ?></td><td><a class="f-link-arrow" href="apply.php?page=course&course=<?= (int)$p['id'] ?>"><?= e(tr('Apply')) ?> →</a></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?>
...
<div class="f-filters f-reveal" role="group" aria-label="<?= e(tr('Notices')) ?>"><input type="search" data-notice-search placeholder="<?= e(tr('Search notices')) ?>" aria-label="<?= e(tr('Search notices')) ?>">
<button class="f-filter-btn" data-cat-filter="all" aria-pressed="true"><?= e(tr('All')) ?> (<?= count($notices) ?>)</button>
<button class="f-filter-btn" data-cat-filter="open" aria-pressed="false"><?= e(tr('Open')) ?> (<?= count(array_filter($notices, fn($x) => !$x['closing'])) ?>)</button>
<button class="f-filter-btn" data-cat-filter="closing" aria-pressed="false"><?= e(tr('Closing soon')) ?> (<?= count(array_filter($notices, fn($x) => $x['closing'])) ?>)</button>
</div>
...
<?php if (!$teachers): ?><p class="f-muted"><?= e(tr('The faculty directory will appear here once the office adds teachers.')) ?></p><?php else: ?><div class="f-grid-3"><?php foreach ($teachers as $t): ?><article class="f-card f-teacher f-reveal"><span class="f-avatar"><?= e(strtoupper(substr($t['name'], 0, 1))) ?></span><div><h3><?= e($t['name']) ?></h3><p class="f-meta"><?= e($t['qualification']) ?></p><p class="f-meta"><?= e($t['institute_name']) ?></p></div></article><?php endforeach; ?></div><?php endif; ?></section>
...
<?php if (!$institutes): ?><p class="f-muted"><?= e(tr('Contact details will appear here soon.')) ?></p><?php else: ?><div class="f-grid-3"><?php foreach ($institutes as $i): ?><article class="f-card f-reveal"><div class="f-ico"><?= collegeIcon('pin') ?></div><h3><?= e($i['name']) ?></h3><?php if (trim($i['address']) !== ''): ?><p><?= collegeIcon('pin') ?> <?= e($i['address']) ?>, <?= e($i['city']) ?></p><?php else: ?><p><?= collegeIcon('pin') ?> <?= e($i['city']) ?></p><?php endif; ?><p><a class="f-link-arrow" href="tel:<?= e(preg_replace('/[^+0-9]/', '', (string)$i['phone'])) ?>"><?= collegeIcon('phone') ?> <?= e($i['phone']) ?></a></p><p><a class="f-btn ghost" href="?page=admissions"><?= e(tr('Admission help')) ?> →</a></p></article><?php endforeach; ?></div><?php endif; ?>
...
<?php if ($allPrograms): ?><div class="f-table-wrap"><table><thead><tr><th>Program</th><th>Institute</th><th>Duration</th><th>Fee</th></tr></thead><tbody><?php foreach ($allPrograms as $p): ?><tr><td><?= e($p['name']) ?></td><td><?= e($p['institute_name']) ?></td><td>₹<?= number_format((int)$p['fee_minor'] / 100, 2) ?></td></tr><?php endforeach; ?></tbody></table></div><?php endif; ?>
...
<p><a class="f-btn ghost" href="?page=login&show=inquiry"><?= e(tr('Admission Inquiry')) ?> →</a></p></aside>
...
<p><a class="f-btn ghost" href="?page=login"><?= e(tr('Back to sign in')) ?> →</a></p></aside>
