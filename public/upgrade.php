...
?>
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>CRM operations upgrade</title><link rel="stylesheet" href="assets/student.css"></head><body><main class="sp-login"><section class="sp-login-intro"><div class="sp-eyebrow">YOUR NEXT CHAPTER</div><h1>More organized operations.<br>Without starting over.</h1><p>Add academic and fee-management modules while preserving your existing institute, admission and payment records.</p></section><section class="sp-login-card"><h2>CRM operations upgrade</h2><?php if($error): ?><div class="sp-alert error"><?=e($error)?></div><?php endif; ?><?php if($success): ?><div class="sp-alert">Upgrade complete. Go to Students, check each personal email, and enable portal access. No student is enabled automatically. Teaching operations, exams, announcements and support are now available.</div><?php endif; ?><p>Signed in as <?=e($user['name'])?>. This adds portal, academic, exam, announcement and support tables. It does not recreate your owner account or replace SMTP settings.</p><p class="sp-small">Portal tables: <?=portalReady()?'ready':'upgrade required'?> · Operations: <?=operationsReady()?'ready':'upgrade required'?> · Student services: <?=servicesReady()?'ready':'upgrade required'?>. Running the upgrade again is safe. Back up first; MySQL schema changes cannot be fully rolled back.</p><form method="post"><?=csrf()?><label><span><input type="checkbox" name="backup" value="yes" required> I have backed up my database and config.php.</span></label><button class="sp-button">Run safe CRM upgrade →</button></form><p class="sp-small"><a href="index.php?page=students">Return to Students →</a> · <a href="student.php">Open student portal ↗</a></p></section></main></body></html>
