...
try {
    ...
    $allowed=['dashboard','institutes','courses','staff','enquiries','followups','admissions','students','settings','audit','payments','documents','notifications','teachers','batches','attendance','fee-plans','fee-reports','health'];
    if ($user && !in_array($page,$allowed,true)) { http_response_code(404); $page='notfound'; }
    if ($user && in_array($page,['staff','audit','payments','notifications','teachers','batches','attendance','fee-plans','fee-reports','health'],true)) requireRole(['owner','admin']);
