...
with tempfile.TemporaryDirectory(prefix='northstar-portal-') as tmp:
    ...
    def login(browser,address):
        clear_limits();master=Master(browser);master.otp_verify(master.otp_start(address),code_for(address))
    ...
    try:
        ...
        step_disabled=Master(a).otp_start('first@example.test')
        check('active student portal account' in step_disabled,'not-enabled email gets generic response')
        check(len(list(capture.glob('*.eml')))==before,'not-enabled student receives no code')
        check('Invalid, expired' in Master(a).otp_verify(step_disabled,'000000'),'unprovisioned student cannot verify')
        ...
        a.get();check(MASTER_MARK in a.html and 'Send sign-in code' in a.html,'student portal address opens the master sign-in')
        check('expired' in Master(a).raw(LOGIN,dict(action='otp_start',csrf='bad',email='first@example.test')),'master sign-in requires CSRF')
        clear_limits();master_a=Master(a);step_a=master_a.otp_start('first@example.test');code=code_for('first@example.test')
        check(code not in a.html,'student OTP never appears in browser response')
        resend_a=master_a.otp_start('first@example.test')
        check('60 seconds' in resend_a,'student resend is throttled')
        check('Your session expired' in Master(b).otp_verify(Master(b).page(),code),'student code is bound to requesting browser')
        wrong_a=master_a.otp_verify(resend_a,'000000')
        check('Invalid, expired' in wrong_a,'incorrect student OTP rejected')
        check('Hello, First' in master_a.otp_verify(wrong_a,code),'student signs in through shared PHPMailer transport')
        check('Your session expired' in master_a.otp_verify(master_a.page(),code),'used student code cannot be replayed')
        ...
        check(MASTER_MARK in a.get('office.php'),'student session cannot authenticate to staff CRM')
        ...
        b.post('logout');check(MASTER_MARK in b.html,'student logout removes access')
        ...
        clear_limits();master_b=Master(b);step_exp=master_b.otp_start('second@example.test');expired=code_for('second@example.test')
        execute('UPDATE portal_otp_challenges SET expires_at=1')
        check('Invalid, expired' in master_b.otp_verify(step_exp,expired),'expired portal OTP rejected')
        clear_limits();step_lock=master_b.otp_start('second@example.test');exhausted=code_for('second@example.test')
        for _ in range(5):step_lock=master_b.otp_verify(step_lock,'000000')
        check('Invalid, expired' in master_b.otp_verify(step_lock,exhausted),'student OTP locks after five wrong attempts')
        staff=Browser(base,'office.php');clear_limits();master_staff=Master(staff);step_staff=master_staff.otp_start('owner@example.test');staff_code=code_for('owner@example.test')
        check('Your session expired' in Master(anon).otp_verify(Master(anon).page(),staff_code),'staff OTP cannot authenticate a student session')
        clear_limits();step_revoke=master_b.otp_start('second@example.test');pending=code_for('second@example.test')
        ...
        check('Invalid, expired' in master_b.otp_verify(step_revoke,pending),'disable then re-enable still invalidates previously issued student OTP')
        owner.post('portal_access',page='students',student_id=sid,access='disable')
        check(MASTER_MARK in a.get(),'disabling portal immediately invalidates an existing session')
        ...
        check(MASTER_MARK in a.get(),'changing email invalidates existing student sessions')
