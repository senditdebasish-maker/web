...
from master import Master, LOGIN, MASTER_MARK, csrf as master_csrf
...
with tempfile.TemporaryDirectory(prefix='northstar-mail-') as tmp:
    ...
    try:
        ...
        owner=Browser(base); stranger=Browser(base); master_owner=Master(owner); master_stranger=Master(stranger)
        check(MASTER_MARK in owner.get() and 'Send sign-in code' in owner.html,'master login replaces the office password form')
        locked=master_owner.page()
        check('Password login is disabled' in master_owner.raw(LOGIN,dict(action='password_login',csrf=master_csrf(locked),email='owner@example.test',password='Test-Owner-Password!')),'password endpoint cannot bypass OTP')
        check('expired' in master_owner.raw(LOGIN,dict(action='otp_start',csrf='bad',email='owner@example.test')),'OTP requests require CSRF')
        step_owner=master_owner.otp_start('owner@example.test')
        check('active staff account' in step_owner,'registered account receives generic success response')
        ...
        resend_owner=master_owner.otp_start('owner@example.test')
        check('60 seconds' in resend_owner,'resend cooldown is enforced')
        check('Your session expired' in master_stranger.otp_verify(master_stranger.page(),code),'code cannot authenticate a different browser session')
        wrong_owner=master_owner.otp_verify(resend_owner,'000000')
        check('Invalid, expired' in wrong_owner,'wrong code is rejected')
        check('Hello, Test' in master_owner.otp_verify(wrong_owner,code),'correct code signs in existing owner')
        check(scalar('SELECT consumed FROM otp_challenges ORDER BY expires_at DESC LIMIT 1')==1,'successful OTP is consumed')
        check('Your session expired' in master_owner.otp_verify(master_owner.page(),code),'used code cannot be replayed')
        owner.post('logout'); unthrottle()
        before=len(messages())
        check('No account found' in master_stranger.otp_start('unknown@example.test'),'unknown email is offered signup or inquiry instead of a code')
        check(len(messages())==before,'unknown email sends no message and creates no account')
        check('Your session expired' in master_stranger.otp_verify(master_stranger.page(),'000000'),'unknown email cannot sign in')
        unthrottle(); step_exp=master_owner.otp_start('owner@example.test'); expired=last_code()
        execute('UPDATE otp_challenges SET expires_at=1')
        check('Invalid, expired' in master_owner.otp_verify(step_exp,expired),'expired code rejected')
        unthrottle(); step_lock=master_owner.otp_start('owner@example.test'); exhausted=last_code()
        for _ in range(5): step_lock=master_owner.otp_verify(step_lock,'000000')
        check('Invalid, expired' in master_owner.otp_verify(step_lock,exhausted),'challenge locks after five wrong attempts')
        unthrottle(); master_owner.otp_start('owner@example.test'); old_id=scalar('SELECT id FROM otp_challenges WHERE consumed=0 ORDER BY expires_at DESC LIMIT 1')
        unthrottle(); step_fresh=master_owner.otp_start('owner@example.test')
        check(scalar('SELECT consumed FROM otp_challenges WHERE id=?',(old_id,))==1,'resending invalidates previous challenge')
        master_owner.otp_verify(step_fresh,last_code())
        ...
        unthrottle(); beta=Browser(base); master_beta=Master(beta); master_beta.otp_verify(master_beta.otp_start('beta-admin@example.test'),last_code('beta-admin@example.test'))
        ...
        unthrottle(); other=Browser(base); master_other=Master(other); master_other.otp_verify(master_other.otp_start('counsellor2@example.test'),last_code('counsellor2@example.test'))
        ...
        unthrottle(); ownCounsellor=Browser(base); master_own=Master(ownCounsellor); master_own.otp_verify(master_own.otp_start('counsellor1@example.test'),last_code('counsellor1@example.test'))
        ...
        unthrottle(); disabled=Browser(base); master_disabled=Master(disabled); step_disabled=master_disabled.otp_start('counsellor1@example.test'); pending_code=last_code('counsellor1@example.test')
        owner.post('staff_toggle',page='staff',id=ac)
        check('Invalid, expired' in master_disabled.otp_verify(step_disabled,pending_code),'disabled user cannot redeem previously issued OTP')
        write_config(environment='production'); unthrottle()
        check('not configured' in master_stranger.otp_start('owner@example.test'),'mail capture cannot enable OTP login in production')
        write_config()
