...
with tempfile.TemporaryDirectory(prefix='northstar-applications-') as temp:
    ...
    try:
        ...
        fields=dict(name='Online Applicant <script>',phone='9000000000',city='Baharampur',qualification='Higher secondary science',completion_year='2025',note='Please explain the next steps.',consent='yes',father_name='Father Applicant',mother_name='Mother Applicant',date_of_birth='2005-06-01',gender='Female',category='General',nationality='Indian',board_10='W.B.B.S.E.',year_10='2021',roll_10='R10TEST',total_10='700',obtained_10='605',board_12='W.B.C.H.S.E.',year_12='2023',stream_12='Science',roll_12='R12TEST',total_12='500',obtained_12='413')
        ...
        check('father&#039;s name' in submit(visitor,father_name=''),'wizard requires father name')
        check('Select your gender' in submit(visitor,gender='Unknown'),'wizard rejects unknown gender')
        check('Select your category' in submit(visitor,category='Unknown'),'wizard rejects unknown category')
        check('not above the total' in submit(visitor,obtained_10='701'),'wizard rejects marks above total')
        check('12-digit Aadhaar' in submit(visitor,aadhaar='123'),'wizard rejects short Aadhaar number')
        check('Class 10 passing year' in submit(visitor,year_10='1800'),'wizard rejects impossible board year')
        ...
        check('Father Applicant' in visitor.html and '86.43' in visitor.html and '82.60' in visitor.html,'acknowledgement shows family and computed board percentages')
        check(scalar("SELECT json_extract(data_json,'$.percentage_10') FROM admission_applications WHERE id=?",(appid,))=='86.43','server computes Class 10 percentage from marks')
        check(scalar("SELECT json_extract(data_json,'$.percentage_12') FROM admission_applications WHERE id=?",(appid,))=='82.60','server computes Class 12 percentage from marks')
        wizard_html=visitor.get('apply.php?page=apply&course='+str(cid))
        seg=wizard_html.split('<ol class="w-steps"')[1].split('</ol>')[0]
        check(seg.count('data-wdot')==4 and 'Registration' not in seg,'form is a four-step wizard without registration')
        check('data-wstep="4"' in wizard_html and 'Preview &amp; Submit' in wizard_html and 'I hereby declare' in wizard_html,'wizard ends in preview with declaration')
        check('name="board_10"' in wizard_html and 'name="stream_12"' in wizard_html and 'assets/wizard.js' in wizard_html,'wizard carries board fields and step script')
        check('name="doc_photo"' not in wizard_html and 'not enabled for this installation' in wizard_html,'wizard hides document slots when storage is off')
        key=scalar('SELECT request_key FROM admission_applications')
        check('already submitted' in visitor.post('submit_application',page='dashboard',request_key=key,course_id=cid,**fields),'duplicate submission returns existing acknowledgement')
        ...
        check(scalar('SELECT status FROM admission_applications')=='Pending Review','resubmission returns application to submitted queue')
        ...
        check(scalar('SELECT COUNT(*) FROM students')==2 and scalar('SELECT status FROM admission_applications')=='Pending Review' and scalar('SELECT COUNT(*) FROM portal_accounts')==0,'failed approval rolls back student, state and portal account')
        ...
        # Organized staff review: Approve, Reject, Revert back, Cancel + Accepted subtab.
        execute('DELETE FROM auth_events')
        reviewer=Browser(base,'apply.php');master_reviewer=Master(reviewer);master_reviewer.applicant_verify(master_reviewer.applicant_start('review1@example.test'),code_for('review1@example.test'))
        check('Application saved' in submit(reviewer,name='Review One',gender='Female',father_name='Father One',board_university='State Board',percentage='82.5',entrance_exam='NEET',entrance_rank='1234'),'rich application form submits')
        review1=scalar('SELECT MAX(id) FROM admission_applications');ref1=scalar('SELECT reference FROM admission_applications WHERE id=?',(review1,))
        view=owner.get('office.php?page=applications&application='+str(review1))
        check('Personal details' in view and 'Contact and family' in view and 'Educational background' in view and 'Course and entrance details' in view,'review page organizes entered data into sections')
        check('Father One' in view and 'State Board' in view and '82.5' in view and 'NEET' in view and '1234' in view,'review page shows all entered form data')
        check('approve_application' in view and '>Approve <span>' in view and 'reject_application' in view and '>Reject <span>' in view,'review page offers Approve and Reject')
        check('request_application_revision' in view and '>Revert back <span>' in view and 'cancel_application' in view and '>Cancel <span>' in view,'review page offers Revert back and Cancel')
        check('Suspend applicant sign-in' not in view and 'Certificate verification' not in view and 'Eligibility automation history' not in view,'review page drops sign-in, certificate and automation sections')
        check('Applicant email alerts' not in view and 'Latest 100' not in view,'review page drops mail alerts and history sections')
        check('Changes saved' in owner.post('request_application_revision',page='applications',application_id=review1,version='1',message='Fix the percentage.'),'staff reverts application for corrections')
        check(scalar('SELECT status FROM admission_applications WHERE id=?',(review1,))=='Revision','reverted application waits for applicant')
        check('Fix the percentage.' in reviewer.get('apply.php?page=application&id='+str(review1)),'applicant sees revert note')
        check('Correct and resubmit' in reviewer.html and 'data-wstep="4"' in reviewer.html,'reverted applicant gets the wizard correction form')
        check('Application saved' in reviewer.post('revise_application',page='dashboard',application_id=review1,version='2',**(fields|{'name':'Review One','qualification':'Higher secondary'})),'applicant resubmits after revert')
        check('Changes saved' in owner.post('approve_application',page='applications',application_id=review1,version='3',approval='yes'),'staff approves reverted application')
        check(scalar('SELECT status FROM admission_applications WHERE id=?',(review1,))=='Approved','approval records Approved state')
        new_portal_sid=scalar('SELECT student_id FROM admission_applications WHERE id=?',(review1,))
        check(scalar('SELECT active FROM portal_accounts WHERE student_id=?',(new_portal_sid,))==1,'approval enables student portal access')
        accepted=owner.get('office.php?page=applications&tab=accepted')
        check('Accepted applications' in accepted and ref1 in accepted,'accepted subtab lists the approved application')
        execute('DELETE FROM auth_events')
        reviewer2=Browser(base,'apply.php');master_reviewer2=Master(reviewer2);master_reviewer2.applicant_verify(master_reviewer2.applicant_start('review2@example.test'),code_for('review2@example.test'))
        check('Application saved' in submit(reviewer2,name='Review Two'),'second review applicant submits')
        review2=scalar('SELECT MAX(id) FROM admission_applications');ref2=scalar('SELECT reference FROM admission_applications WHERE id=?',(review2,))
        check('Changes saved' in owner.post('reject_application',page='applications',application_id=review2,version='1',message='Seats are full.'),'staff rejects application')
        check(scalar('SELECT status FROM admission_applications WHERE id=?',(review2,))=='Rejected','rejection records Rejected state')
        check('Seats are full.' in reviewer2.get('apply.php?page=application&id='+str(review2)),'applicant sees rejection reason')
        execute('DELETE FROM auth_events')
        reviewer3=Browser(base,'apply.php');master_reviewer3=Master(reviewer3);master_reviewer3.applicant_verify(master_reviewer3.applicant_start('review3@example.test'),code_for('review3@example.test'))
        check('Application saved' in submit(reviewer3,name='Review Three'),'third review applicant submits')
        review3=scalar('SELECT MAX(id) FROM admission_applications');ref3=scalar('SELECT reference FROM admission_applications WHERE id=?',(review3,))
        check('Confirm eligibility' in owner.post('approve_application',page='applications',application_id=review3,version='1'),'approval requires eligibility confirmation')
        check('Changes saved' in owner.post('cancel_application',page='applications',application_id=review3,version='1',message='Duplicate entry.'),'staff cancels application')
        check(scalar('SELECT status FROM admission_applications WHERE id=?',(review3,))=='Withdrawn','cancelled application closes as withdrawn')
        check('cancelled by the office' in reviewer3.get('apply.php?page=application&id='+str(review3)),'applicant sees cancellation note')
        check('already closed' in owner.post('reject_application',page='applications',application_id=review3,version='2',message='Late'),'closed application rejects further decisions')
        accepted2=owner.get('office.php?page=applications&tab=accepted')
        check(ref1 in accepted2 and ref2 not in accepted2 and ref3 not in accepted2,'accepted subtab holds only approved applications')
