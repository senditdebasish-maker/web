...
with tempfile.TemporaryDirectory(prefix='northstar-student-accounts-') as temp:
    ...
    try:
        ...
        check('valid 10-digit mobile' in master_s.create_start(**dict(newbie,phone='bad')),'invalid phone rejected')
...
>>>>>>> parent of ba104b3 (new)
        ...
        check('YOUR STUDENT SPACE' in master_t.create_verify(step_t,code_for('newstudent@example.test')),'existing email signs in with a fresh code')
        ...
        check('YOUR STUDENT SPACE' not in s.get('student.php') and MASTER_MARK in s.html,'disabled account loses access immediately')
        ...
        check('YOUR STUDENT SPACE' in master_s.create_verify(step_back,code_for('newstudent@example.test')),'restored account verified')
        ...
        check('Enrolled' in s.get('student.php') and 'YOUR STUDENT SPACE' not in s.html,'verified account links automatically to the full student portal')
