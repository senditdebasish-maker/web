...
?>
<div class="alert success">Record cash/counter payments here. Online Razorpay orders appear below; only Captured + verified payments credit the ledger. Manual entry is blocked while an online order is pending.</div>
...
<?php if(function_exists('automationReady')&&automationReady()):$ow=$scope?'o.institute_id=?':'1=1';$online=rows('SELECT o.*,s.name student_name,i.name institute_name FROM online_orders o JOIN students s ON s.id=o.student_id JOIN institutes i ON i.id=o.institute_id WHERE '.$ow.' ORDER BY o.id DESC LIMIT 20',$args);?>
<section class="panel"><div class="panel-heading"><div><h2>Online payment orders</h2><p>Verify in the Razorpay dashboard before reconciling. Test orders never credit the ledger. Webhook: <code>public/razorpay-webhook.php?institute=&lt;id&gt;</code>.</p></div></div><?php if(!$online):?><div class="empty"><p>No online orders yet. Students create orders from their portal when their institute is configured.</p></div><?php else:table(['Order / student','Amount / mode','State / receipt','Captures','Reconcile'],$online,function($o){
$caps=rows('SELECT * FROM online_captures WHERE order_id=?',[$o['id']]);$capHtml=$caps?implode('<br>',array_map(fn($c)=>'<small>'.e($c['provider_payment']).' · '.e($c['state']).($c['payment_id']?' · RCPT-'.str_pad((string)$c['payment_id'],7,'0',STR_PAD_LEFT):'').'</small>',$caps)):'<small>No captured payment yet</small>';
$act='<details class="row-details"><summary>Reconcile →</summary><form method="post" class="stack">'.csrf().'<input type="hidden" name="action" value="reconcile_order"><input type="hidden" name="id" value="'.$o['id'].'"><label>Razorpay order ID (for Uncertain)<input name="provider_order" maxlength="100" value="'.e((string)$o['provider_order']).'"></label><label><span><input type="checkbox" name="allow_review" value="yes"> Allow crediting a Review capture after manual verification</span></label>'.($o['state']==='Closed'?'<label><span><input type="checkbox" name="reopen" value="yes"> Confirm reopening this closed order</span></label>':'').'<button class="button secondary">Verify with gateway</button></form></details>';
if(!in_array($o['state'],['Paid','TestPaid'],true))$act.='<details class="row-details"><summary>Close order</summary><form method="post" class="stack">'.csrf().'<input type="hidden" name="action" value="close_online_order"><input type="hidden" name="id" value="'.$o['id'].'"><label>Reason<textarea name="reason" maxlength="300" rows="2" required></textarea></label><button class="button secondary">Close without credit</button></form></details>';
return [person('#'.$o['id'].' · '.$o['student_name'],$o['institute_name']),'<strong>'.money((int)$o['amount_minor']).'</strong><small>'.e($o['mode']).' · '.e($o['key_id']).'</small>',badge($o['state']).'<small>'.e($o['receipt']).'<br>'.e((string)$o['provider_order']).'</small><small>'.e($o['note']).'</small>',$capHtml,$act];});endif;?></section><?php endif;?>
<?php
    [$pw,$pa]=scoped('p');
