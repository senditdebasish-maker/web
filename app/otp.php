...
function requestOtp(): string {
    if (!otpEnabled()) fail('Email-code login is not enabled.');
    $email=emailInput();
    ...
    try {
        ...
        query('DELETE FROM otp_challenges WHERE expires_at<?',[$now-86400]);
        ...
        query('UPDATE otp_challenges SET consumed=1 WHERE email_hash=?',[$identity]);
        query('INSERT INTO otp_challenges (id,user_id,email_hash,code_hash,expires_at) VALUES (?,?,?,?,?)',[$id,$user['id'] ?? null,$identity,hash_hmac('sha256',$code,$secret),$now+300]);
        db()->commit();
    } catch (Throwable $e) { if(db()->inTransaction()) db()->rollBack(); throw $e; }
    $_SESSION['otp']=['id'=>$id,'secret'=>$secret,'email'=>$email];
    if ($user) {
        try {
            sendMail($email,'Your Northstar sign-in code',"Your sign-in code is: $code\n\nThis code expires in 5 minutes and can be used once, in the same browser that requested it. Never share this code.\n\nIf you did not request it, ignore this email.");
        } catch (Throwable $e) {
    ...
    $_SESSION['flash']='If this email belongs to an active staff account, a code has been sent. Check your inbox and spam folder. It expires in 5 minutes.';
    return 'login';
}
function verifyOtp(): string {
    if (!otpEnabled()) fail('Email-code login is not enabled.');
    $pending=$_SESSION['otp'] ?? null;
    if (!$pending) fail('Request a new sign-in code first.');
    $code=input('code',20);
    $ip=hash('sha256',$_SERVER['REMOTE_ADDR'] ?? 'unknown');
    writeTransaction();
    try {
        $challenge=one('SELECT * FROM otp_challenges WHERE id=?'.lockSuffix(),[$pending['id']]);
        ...
        query("INSERT INTO auth_events (identity_hash,ip_hash,kind,attempted_at) VALUES (?,?,'verify',?)",[hash('sha256',$pending['email']),$ip,time()]);
        $valid=$challenge && !$challenge['consumed'] && (int)$challenge['expires_at']>time() && (int)$challenge['attempts']<5;
        if ($valid) {
            query('UPDATE otp_challenges SET attempts=attempts+1 WHERE id=?',[$challenge['id']]);
            $valid=preg_match('/^\d{6}$/D',$code) && hash_equals($challenge['code_hash'],hash_hmac('sha256',$code,$pending['secret']));
        }
        $user=$valid ? one('SELECT * FROM users WHERE id=? AND active=1',[$challenge['user_id']]) : null;
        if ($user) query('UPDATE otp_challenges SET consumed=1 WHERE id=?',[$challenge['id']]);
        db()->commit();
    } catch (Throwable $e) { if(db()->inTransaction()) db()->rollBack(); throw $e; }
    if (!$user) fail('Invalid, expired, or exhausted code. Request a new code if needed.');
    session_regenerate_id(true);
    $_SESSION=['uid'=>(int)$user['id'],'csrf'=>bin2hex(random_bytes(32)),'last_seen'=>time()];
    audit('otp_login','users',(int)$user['id']);
