...
function requestOtp(string $audience='staff'): string {
    ...
    try {
        ...
        $user=$student ? (portalAccountByEmail($email) ?: one('SELECT * FROM student_users WHERE LOWER(email)=? AND active=1',[$email]) ?: one('SELECT * FROM applicant_accounts WHERE LOWER(email)=? AND active=1',[$email])) : one('SELECT * FROM users WHERE LOWER(email)=? AND active=1',[$email]);
        $portalUser=$student ? portalAccountByEmail($email) : null;
        query("UPDATE $table SET consumed=1 WHERE email_hash=?",[$identity]);
        query("INSERT INTO $table (id,user_id,email_hash,code_hash,expires_at".($student?',access_version':'').') VALUES (?,?,?,?,?'.($student?',?':'').')',array_merge([$id,$student ? ($portalUser['id'] ?? null) : ($user['id'] ?? null),$identity,hash_hmac('sha256',$code,$secret),$now+300],$student?[(int)($user['access_version'] ?? $user['version'] ?? 0)]:[]));
        db()->commit();
...
function verifyOtp(string $audience='staff'): string {
    ...
    try {
        ...
        $user=$valid ? ($student ? (portalAccountById((int)$challenge['user_id']) ?: one('SELECT * FROM student_users WHERE email=? AND active=1',[$pending['email']]) ?: one('SELECT * FROM applicant_accounts WHERE email=? AND active=1',[$pending['email']])) : one('SELECT * FROM users WHERE id=? AND active=1',[$challenge['user_id']])) : null;
        if ($student && $user && (int)($user['access_version'] ?? $user['version'])!==(int)$challenge['access_version']) $user=null;
    ...
    if ($student) {
        ...
        } elseif (array_key_exists('version',$user) && array_key_exists('email',$user) && !array_key_exists('phone',$user)) {
            global $config;
            $_SESSION['applicant_id']=(int)$user['id']; $_SESSION['applicant_version']=(int)$user['version']; $_SESSION['applicant_email']=$user['email']; $_SESSION['applicant_site']=hash('sha256',__DIR__.$config['dsn']);
        } else {
            $_SESSION['suid']=(int)$user['id']; $_SESSION['sversion']=(int)$user['version'];
