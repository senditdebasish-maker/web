...
if(in_array($page,['staff','audit','payments','notifications'],true) && !$admin) { http_response_code(403); $page='forbidden'; }
...
if($admin) { $nav['payments']='Payments'; $nav['notifications']='Email notifications'; $nav['staff']='Team & access'; }
$titles=['payments'=>'Record a payment. Keep a clear receipt.','documents'=>'Admission letters and payment receipts.','notifications'=>'A clear view of every student email.','dashboard'=>'A little clarity. A lot of progress.','enquiries'=>'Every conversation, a possibility.','followups'=>'Make the next connection.','admissions'=>'Their next chapter starts here.','students'=>'Your growing student community.','courses'=>'Learning paths, organized.','institutes'=>'One workspace. Every institute.','staff'=>'Good work starts with a great team.','settings'=>'Make yourself at home.','audit'=>'A clear record of activity.'];
...
<?php elseif($page==='settings'): ?><section class="panel settings-panel"><div class="panel-heading"><div><h2>Account & security</h2><p><?=e($user['email'])?> · <?=e(ucfirst($user['role']))?></p></div></div><div class="settings-body"><?php if(otpEnabled()): ?><div class="auth-method">✉ Email OTP enabled</div><p class="inline-note">Sign in using a five-minute code delivered to your registered staff email. Password login is disabled. Contact the server administrator if you cannot access your mailbox.</p><?php else: formStart('password'); field('Current password','current_password','','password'); field('New password (12+ characters)','password','','password'); field('Confirm new password','confirm_password','','password'); formEnd('Update password'); endif; ?></div></section>
...
<footer class="app-footer"><span><b>northstar.</b> A space for better beginnings.</span><span>Institute CRM · Email & receipts</span></footer></main></div>
