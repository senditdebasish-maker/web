...
$listingId=(int)($_GET['listing']??0);$policyId=(int)($_GET['policy']??0);
if($listingId){$course=record('courses',$listingId);$listing=one('SELECT * FROM admission_listings WHERE id=?',[$listingId]);?><section class="panel editor"><div class="panel-heading"><div><h2>Public admission listing</h2><p>Configure what applicants see in the public admission form.</p></div><a class="close" href="?page=admissions">Back</a></div><?php formStart('admission_listing');?><input type="hidden" name="course_id" value="<?=$listingId?>"><input type="hidden" name="version" value="<?=(int)($listing['version']??0)?>"><?php textArea('Public course description','description',$listing['description']??'',true);textArea('Eligibility and original documents to verify at the office','eligibility',$listing['eligibility']??'',true);textArea('Institute privacy notice','privacy_notice',$listing['privacy_notice']??'',true);?><div class="form-grid"><?php field('Open from','opens_on',$listing['opens_on']??date('Y-m-d'),'date');field('Close after','closes_on',$listing['closes_on']??date('Y-m-d',strtotime('+30 days')),'date');selectField('Public applications','accepting',['0'=>'Closed / hidden','1'=>'Open within date window'],$listing['accepting']??'0');?></div><?php formEnd('Save public listing');?></section><?php } elseif($policyId){$course=record('courses',$policyId);$policy=automationReady()?one('SELECT * FROM eligibility_policies WHERE id=?',[$policyId]):null;?><section class="panel editor"><div class="panel-heading"><div><h2>Eligibility automation</h2><p>Owner-authorized rules for certificate verification.</p></div><a class="close" href="?page=admissions">Back</a></div><?php if($user['role']!=='owner'):?><p class="inline-note">Only the group owner can authorize automatic admission.</p><?php elseif(!automationReady()):?><p class="inline-note">Run the automation upgrade first.</p><?php else:formStart('eligibility_policy');?><input type="hidden" name="course_id" value="<?=$policyId?>"><input type="hidden" name="version" value="<?=(int)($policy['version']??0)?>"><div class="form-grid"><?php selectField('Automatic admission','enabled',['0'=>'Disabled (manual review)','1'=>'Enabled for matching verified evidence'],(string)($policy['enabled']??'0'));field('Required qualification code','qualification_code',$policy['qualification_code']??'');field('Minimum percentage','minimum_percentage',isset($policy['minimum_percentage'])?number_format($policy['minimum_percentage']/100,2,'.',''):'');field('Minimum age','minimum_age',(string)($policy['minimum_age']??''));field('Maximum age','maximum_age',(string)($policy['maximum_age']??''));field('Age cutoff date','cutoff_on',$policy['cutoff_on']??date('Y-m-d'),'date');?></div><?php textArea('Owner authorization note','description',$policy['description']??'',true);?><label><span><input type="checkbox" name="confirm" value="yes" required> I confirm these are the approved course rules.</span></label><?php formEnd('Save eligibility policy');endif;?></section><?php } else {echo '<div class="list-toolbar admissions-actions"><span class="subtle-tag">Admissions management</span><a class="button secondary" href="#public-listings">Manage public course listings</a><a class="button secondary" href="#eligibility">Eligibility automation</a></div>';
$publicListings=rows('SELECT c.id,c.name,c.active,l.accepting,l.opens_on,l.closes_on FROM courses c LEFT JOIN admission_listings l ON l.id=c.id WHERE '.($scope?'c.institute_id=?':'1=1').' ORDER BY c.name',$scope?[$scope]:[]);echo '<section class="panel"><div class="panel-heading"><div><h2>Public course listings</h2><p>Courses shown in the public admission form.</p></div></div>';table(['Course','Course status','Public listing','Admission dates',''],$publicListings,fn($r)=>[e($r['name']),badge($r['active']?'Active':'Archived'),badge($r['accepting']?'Open':'Closed'),e(($r['opens_on']??'—').' → '.($r['closes_on']??'—')),'<a class="text-link" href="?page=admissions&listing='.(int)$r['id'].'">Configure →</a>']);echo '</section>';
if(function_exists('automationReady')&&automationReady()){$policies=rows('SELECT c.id,c.name,c.active,p.enabled,p.version policy_version FROM courses c LEFT JOIN eligibility_policies p ON p.id=c.id WHERE '.($scope?'c.institute_id=?':'1=1').' ORDER BY c.name',$scope?[$scope]:[]);echo '<section class="panel"><div class="panel-heading"><div><h2>Eligibility automation</h2><p>Owner-authorized certificate rules for public applications.</p></div></div>';table(['Course','Course status','Automation','Policy',''],$policies,fn($r)=>[e($r['name']),badge($r['active']?'Active':'Archived'),badge($r['enabled']?'Enabled':'Disabled'),'v'.(int)($r['policy_version']??0),'<a class="text-link" href="?page=admissions&policy='.(int)$r['id'].'">Review →</a>']);echo '</section>';}}
elseif(in_array($page,['enquiries','students','followups','courses','institutes','staff','audit'],true)):
