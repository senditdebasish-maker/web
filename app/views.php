...
<?php formStart('login'); field('Email address','email','','email'); field('Password','password','','password'); formEnd('Sign in to workspace'); ?>
<p class="login-note">Need an account? Contact your institute administrator.<br>First time setting up? Follow the README installation guide.</p></main></div>
<?php else:
try {
$admin=in_array($user['role'],['owner','admin'],true);
if(in_array($page,['staff','audit'],true) && !$admin) { http_response_code(403); $page='forbidden'; }
...
if($page==='institutes') {
    ...
    field('Full name','name'); field('Email address','email','','email'); selectField('Role','role',$user['role']==='owner' ? ['admin'=>'Institute administrator','counsellor'=>'Counsellor'] : ['counsellor'=>'Counsellor']); field('Initial password (12+ characters)','password','','password');
...
} elseif($page==='students') table(['Student','Student ID','Institute & course','Admitted','Agreed course fee'], $data,fn($r)=>[person($r['name'],$r['phone']),'<span class="mono">ST-'.str_pad((string)$r['id'],5,'0',STR_PAD_LEFT).'</span>','<strong>'.e($r['course_name']).'</strong><small>'.e($r['institute_name']).'</small>',e($r['admission_date']),money((int)$r['fee_minor'])]);
...
elseif($page==='staff') table(['Team member','Institute','Role','Access',''],$data,function($r)use($user){
    ...
    return [person($r['name'],$r['email']),e($r['institute_name']),badge(ucfirst($r['role'])),badge($r['active']?'Active':'Disabled'),$can?'<form method="post">'.csrf().'<input type="hidden" name="action" value="staff_toggle"><input type="hidden" name="id" value="'.$r['id'].'"><button class="text-button">'.($r['active']?'Disable access':'Restore access').'</button></form>':''];
});
...
<?php if($page==='students'): ?><p class="footnote">Course fees are recorded at admission. Payment collection, receipts, attendance, and exams are planned for the next milestone.</p><?php endif; ?>
<?php elseif($page==='settings'): ?><section class="panel settings-panel"><div class="panel-heading"><div><h2>Account & security</h2><p><?=e($user['email'])?> · <?=e(ucfirst($user['role']))?></p></div></div><div class="settings-body"><?php formStart('password'); field('Current password','current_password','','password'); field('New password (12+ characters)','password','','password'); field('Confirm new password','confirm_password','','password'); formEnd('Update password'); ?></div></section>
...
<footer class="app-footer"><span><b>northstar.</b> A space for better beginnings.</span><span>Institute CRM · Milestone 1</span></footer></main></div>
